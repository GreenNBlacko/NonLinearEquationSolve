...
namespace CS_IMGUI.GUI {
	public class Renderer : Overlay {
		...
		public void LoadMenu(e_Menus menuID, params object?[] args) {
			if (args.Length > 0) {
				menu = Activator.CreateInstance(menus[(int)menuID], ctx, args) as Menu ?? throw new ArgumentNullException("Menu does not exist");
				return;
			}

			menu = Activator.CreateInstance(menus[(int)menuID], ctx) as Menu ?? throw new ArgumentNullException("Menu does not exist");
		}
		...
		private void CrawlMenus() {
			...
			var menuTypes = assembly.GetTypes()
				.Where(t => t.IsSubclassOf(typeof(Menu)) && !t.IsAbstract);

			foreach (var menuType in menuTypes) {
				try {
					var instance = Activator.CreateInstance(menuType, ctx) as Menu;
					if (instance != null && instance.priority >= 0) {
						menus.Add(menuType);
					}
				} catch { }
			}

			menus.Sort((a, b) => { return (Activator.CreateInstance(a, ctx) as Menu).priority > (Activator.CreateInstance(b, ctx) as Menu).priority ? 1 : -1; });
		}
	}
}
